...
def findbox(depth_frame):
    ...
    depth_data = np.where((depth_data > 1550), 3000, depth_data)
    ...
    peri = cv2.arcLength(max_contour, True)  # 周长
    box = cv2.approxPolyDP(max_contour, 0.07 * peri, True)  # 轮廓角点坐标
    if len(box) != 4:
        print(box.shape)
        return None
    # box_img = np.zeros((480, 640, 3), dtype=np.int8)
    # cv2.drawContours(box_img, [box], -1, (0, 255, 0), 2)
